...
def parse_args():
    ...
    parser.add_argument("--h", type=int, default=360, required=False)
    parser.add_argument("--w", type=int, default=900, required=False)
    ...
    parser.add_argument("--checkpoints", type=str, default='v6', required=False)
...
def split_data(fold, use_extra=True):
    ...
    if use_extra:
        data_extra = pd.read_csv('/data/user_data/mkdata.csv').sample(frac=0.5, random_state=fold)
    ...
    for epoch in range(1, epochs):
        ...
        for i, (inputs, labels) in enumerate(train_data):
            ...
            with autocast():
                ...
                if enhance_prob < 0.2:
                    inputs, target_a, target_b, lam = mixup(inputs, labels, alpha=0.5)
                    outputs = model(inputs)
                    loss = loss_func(outputs, target_a) * lam + (1 - lam) * loss_func(outputs, target_b)
...
if __name__ == '__main__':
    ...
    Width = args.w
    Height = args.h
    # train_mean, train_std = init_normalize(train_dir, size=[Height, Width])
    train_mean, train_std = [0.8408691800077757, 0.843093117582798, 0.8397223384737968], [
        0.20037801617731651, 0.19797363897711037, 0.20214536744058131]  # 360 900
    # train_mean, train_std = [0.8448635504603386, 0.8477284370994568, 0.8426760819864273][
    #     0.19310236097380518, 0.18947888918563724, 0.19577497842028738] #280 700
    # train_mean, train_std = [0.8408704800764719, 0.8430948053002357, 0.839723514576753], [
    #     0.20036302471856277, 0.19795909674788514, 0.20213062292287748]  # 280 700
    # train_mean, train_std = [0.840869569003582, 0.8430936640501022, 0.8397226673841477], [
    #     0.20039658931915957, 0.19799220910718043, 0.20216414181664585] #384
    ...
    train_paths, val_paths = split_data(args.fold)
    # 加载训练数据集，转化成标准格式
