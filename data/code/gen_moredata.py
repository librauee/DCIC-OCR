...
fonts_path = '/data/user_data/ntailu.ttf'
...
import os 
os.makedirs(name='/data/user_data/mkdata',exist_ok=True)
labels = []
paths  = []
for i in tqdm(range(total_pic)):
    ...
    if label not in labels:
        labels.append(label)
        paths.append(f'/data/user_data/mkdata/{label}.png')
        image.write(label, f'/data/user_data/mkdata/{label}.png')
import pandas as pd 
pd.DataFrame({
    'img_path':paths,
    'label':labels,
    'fold':-1,
}).to_csv('/data/user_data/mkdata.csv',index=False)